...
public class ProgressView extends View {
    ...
    protected void applyStyle(Context context) {
        int mProgressId = MCHInflaterUtils.getIdByName(context,"style","mch_CircularProgress");
    ...
    static class CircularProgressDrawable extends Drawable implements Animatable {
        ...
        public static class Builder {
            ...
            public Builder(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
                ...
                TypedArray a = context.obtainStyledAttributes(attrs, MCHInflaterUtils.getStyleableIntArray(context,"mch_CircularProgressDrawable"), defStyleAttr, defStyleRes);
                int resId;

                padding(a.getDimensionPixelSize(getLL("mch_cpd_padding"), 0));
                initialAngle(a.getInteger(getLL("mch_cpd_initialAngle"), 0));
                maxSweepAngle(a.getInteger(getLL("mch_cpd_maxSweepAngle"), 270));
                minSweepAngle(a.getInteger(getLL("mch_cpd_minSweepAngle"), 1));
                strokeSize(a.getDimensionPixelSize(getLL("mch_cpd_strokeSize"), ThemeUtil.dpToPx(context, 4)));
                strokeColors(a.getColor(getLL("mch_cpd_strokeColor"), ThemeUtil.colorPrimary(context, 0xFF000000)));
                if ((resId = a.getResourceId(getLL("mch_cpd_strokeColors"), 0)) != 0) {
                ...
                reverse(a.getBoolean(getLL("mch_cpd_reverse"), false));
                rotateDuration(a.getInteger(getLL("mch_cpd_rotateDuration"), context.getResources().getInteger(android.R.integer.config_longAnimTime)));
                transformDuration(a.getInteger(getLL("mch_cpd_transformDuration"), context.getResources().getInteger(android.R.integer.config_mediumAnimTime)));
                keepDuration(a.getInteger(getLL("mch_cpd_keepDuration"), context.getResources().getInteger(android.R.integer.config_shortAnimTime)));
                if ((resId = a.getResourceId(getLL("mch_cpd_transformInterpolator"), 0)) != 0)
                    transformInterpolator(AnimationUtils.loadInterpolator(context, resId));
                progressMode(a.getResourceId(getLL("mch_pv_progressMode"), ProgressView.MODE_INDETERMINATE));
                //progressMode(a.getInteger(R.styleable.CircularProgressDrawable_pv_progressMode, ProgressView.MODE_INDETERMINATE));
                inAnimDuration(a.getInteger(getLL("mch_cpd_inAnimDuration"), context.getResources().getInteger(android.R.integer.config_mediumAnimTime)));
                if ((resId = a.getResourceId(getLL("mch_cpd_inStepColors"), 0)) != 0) {
                    TypedArray ta = context.getResources().obtainTypedArray(resId);
                    int[] colors = new int[ta.length()];
                    for (int j = 0; j < ta.length(); j++)
                        colors[j] = ta.getColor(j, 0);
                ...
                inStepPercent(a.getFloat(getLL("mch_cpd_inStepPercent"), 0.5f));
                outAnimDuration(a.getInteger(getLL("mch_cpd_outAnimDuration"), context.getResources().getInteger(android.R.integer.config_mediumAnimTime)));
                a.recycle();
            }

            public int getLL(String name){
             return MCHInflaterUtils.getStyleableFieldId(context,"mch_CircularProgressDrawable",name);
            }
    ...
    static class ThemeUtil {
        ...
        public static int colorPrimary(Context context, int defaultValue) {
            ...
            return getColor(context,
                    MCHInflaterUtils.getIdByName(context,"attr","colorPrimary"), defaultValue);
        }
    }
}