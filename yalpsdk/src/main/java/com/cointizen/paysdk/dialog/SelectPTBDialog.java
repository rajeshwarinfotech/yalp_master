...
public class SelectPTBDialog extends DialogFragment {
    ...
    public SelectPTBDialog() {
    }

    @SuppressLint("ValidFragment")
    public SelectPTBDialog(Context con) {
        this.con = con;
    }
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        try {
            containerView = inflater.inflate(MCHInflaterUtils.getIdByName(con,
                    "layout", "mch_dialog_select_ptb"), container, false);
            // 如果有信息,显示加载信息
            Bundle bundle = getArguments();
            if (bundle == null) {
                return containerView;
            }
            isGamePtb = false;  //默认是否是绑币
...
//		window.getAttributes().width = (int) 400;
    ...
    public static class Builder {
        ...
        private SelectPTBDialog create(Context con) {
            final SelectPTBDialog dialog = new SelectPTBDialog(con);
            // 1,设置显示内容
        ...
        public SelectPTBDialog show(Context con, FragmentManager fm, DiscountPrice discountPrice) {
            if (fm == null) {
            ...
            SelectPTBDialog dialog = create(con);
