...
public class DeviceOnlineRequest {
	...
	private static final String TAG = "DeviceOnlineRequest";
	...
	public void post(String url, RequestParams params, final Context context) {
		...
		http.send(HttpRequest.HttpMethod.POST, url, params, new RequestCallBack<String>() {
			...
			public void onSuccess(ResponseInfo<String> responseInfo) {
				...
				MCLog.e(TAG, HttpConstants.Log_SOMAAYIFxK + response);
				try {
					JSONObject obj = new JSONObject(response);
					int status = obj.getInt("code");
					if (status == 200) {
						Constant.deviceIsOnLine = true;
						MCLog.i(TAG, HttpConstants.Log_aILNiDQqob);
					} else {
						MCLog.e(TAG, HttpConstants.Log_NhXKIPbBpi);
					}
