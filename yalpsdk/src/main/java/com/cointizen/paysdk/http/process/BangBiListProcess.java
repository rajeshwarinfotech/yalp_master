...
import com.cointizen.paysdk.http.request.BangBiListRequest;
 ...
 * 绑币列表
 */
public class BangBiListProcess {

	private static final String TAG = "BangBiListProcess";
	...
	public void post(Handler handler, Context activity) {
		if (null == handler) {
		...
		if (null != params) {
			BangBiListRequest loginRequest = new BangBiListRequest(handler);
			loginRequest.post(MCHConstant.getInstance().getBangBiList(), params,activity);
		} else {
