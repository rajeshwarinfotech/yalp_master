...
import com.cointizen.paysdk.http.request.DeviceOnlineRequest;
 ...
 * 设备上线
 */
public class DeviceOnlineProcess {

	private static final String TAG = "DeviceOnlineProcess";
	private final Context context;

	public DeviceOnlineProcess(final Context context) {
		this.context = context;
	}

	public void post() {
		if(Constant.deviceIsOnLine){
			MCLog.w(TAG,HttpConstants.Log_AjKgoIwqUI);
			return;
		}
	...
	private void sendMsg(){
		...
		if (null != params) {
			DeviceOnlineRequest loginRequest = new DeviceOnlineRequest();
			loginRequest.post(MCHConstant.getInstance().getDeviceOnlineUrl(), params,context);
		} else {
