...
public class FalsifyHeader extends InternalAbstract implements RefreshHeader {
    ...
    protected void dispatchDraw(Canvas canvas) {
        ...
        if (thisView.isInEditMode()) {//这段代码在运行时不会执行，只会在Studio编辑预览时运行，不用在意性能问题
            ...
            textView.setText(thisView.getResources().getString(MCHInflaterUtils.getIdByName(thisView.getContext(),"string","mch_srl_component_falsify"), getClass().getSimpleName(), DensityUtil.px2dp(thisView.getHeight())));
